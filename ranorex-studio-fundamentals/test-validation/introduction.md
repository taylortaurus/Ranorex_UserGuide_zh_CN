...
在软件测试中，验证是验证被测应用程序产生的行为和/或响应是否与预期结果匹配的过程。本章介绍Ranorex Studio中提供的测试验证方法。
...
- 我们只能将显示的结果与用户输入操作的初始序列进行匹配，并确定结果或行为是否符合预期。
...
## 激活验证
在Ranorex中，验证被视为一种特殊类型的操作。因此，“激活”测试验证只是意味着将验证操作添加到当前记录模块。以下说明描述了在记录会话期间添加测试验证。如前所述，还可以在录制后手动添加验证操作。
...
>**贴士**          
录制会话会自动暂停，以允许您插入测试验证操作。这由记录器控制中心的紫色条表示。
...
&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;[基于文本的验证示例👉][4]
