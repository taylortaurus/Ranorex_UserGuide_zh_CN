...
1. 时间
    - 默认情况下，将显示从开始运行测试后的相对执行时间。在[ 👉 报告设置][3]中，默认设置可以更改
2. 级别
    - 第二列显示所执动作作的报告级别
3. 类别
    - 此列显示已执行的动作类型
...
> **章节预览**  
> 在 \> Ranorex Studio 基础教程 \> 测试套件 \> 👉 [执行测试套件][2]章节中详细介绍和解释了`数据迭代和屏蔽数据`的概念
...
### **启用视频报告**
默认情况下禁用视频报告。您可以在[👉报告设置][3]中启用和配置它。
...
[2]: ..\\..\\ranorex-studio-fundamentals/test-suite/running-tests.html
